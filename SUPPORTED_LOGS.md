# Supported Log Patterns in VerseCon Link (v2.7.2)

Generated: 2026-02-20T15:11:14.129Z
...
- **attachment_received**: `/<([^>]+)>\s+\[[^\]]+\]\s+<AttachmentReceived>\s+Player\[([^\]]+)\]\s+Attachment\[([^,\]]+),\s*([^,\]]+),\s*([^\]]+)\][^\n]*?Port\[([^\]]+)\]/i`
- **inventory_management**: `/<InventoryManagement>\s+Request\[(\d+)\]\s+for\s+'([^']+)'\s+\[\d+\]\s+Result\[(\w+)\]/i`
